// dashboard/game-controller.js
// Main game state machine: screen routing, gameplay loop, hint/skip handling

import { getWords, updateWord } from '../utils/storage.js';
import { selectNextWord, nextLearningState, updateStats, calcXP, checkAchievements, maxHintsForLevel } from '../utils/game-engine.js';
import { getProfile, addXP, recordSession, unlockAchievement } from '../utils/player.js';
...
let isSessionActive = false;  // guards nextRound() after endSession()
let gameInitialized = false;  // prevents duplicate event listeners on re-entry
...
export async function initGame() {
  // Refresh vocabulary + profile every time the tab is opened
  words = await getWords();
  const profile = await getProfile();
  maxHints = maxHintsForLevel(profile.level);
  renderMenu(profile);

  // Only show menu if no active session — don't interrupt gameplay
  if (!isSessionActive) showScreen('screen-menu');

  // Attach event listeners only once — prevents duplicate handlers on re-entry
...
async function nextRound() {
  if (!isSessionActive) return; // session ended (e.g. race bar hit 0)
  // words loaded once at startSession; kept in sync via in-memory updates in processResult
  ...
  // Mission: check objectives display
  if (selectedMode === 'mission') renderMissionObjectives(modeInstance?.objectives);
}

function submitAnswer() {
  if (!currentWord || !isSessionActive) return;
  if (!currentWord.text) { skipWord(); return; } // guard against malformed storage entries
  const input = document.getElementById('answer-input');
  const answer = input.value.trim().toLowerCase();
  const correct = answer === currentWord.text.toLowerCase();

  const wordSnapshot = currentWord; // capture before async nulling
  currentWord = null;               // prevent double-submission on fast Enter
...
async function processResult(result, revealWord, word) {
  ...
  if (result.correct) {
    ...
    modeInstance?.onCorrect?.(result.hintUsed); // universal call — handles all modes including mission
  } else {
    feedback.innerHTML = `<span class="feedback-wrong">✗ The answer was: <strong>${revealWord || '(skipped)'}</strong></span>`;
    sessionStats.streak = 0;
    modeInstance?.onWrong?.(); // universal call
  }

  // Update mission objectives display immediately after the mode callback
  if (selectedMode === 'mission') renderMissionObjectives(modeInstance?.objectives);

  sessionStats.totalAnswered += 1;
  if (result.hintUsed) sessionStats.hintCount += 1;

  // Persist learning state + stats — errors must not block the game loop
  try {
    ...
    // Update in-memory array so selectNextWord sees fresh learning states without re-fetching storage
    ...
    console.error('Language Learning: failed to persist result', err);
    feedback.innerHTML += '<span style="color:#f59e0b;font-size:11px;margin-left:6px;">(progress not saved)</span>';
  }

  // Mission: check completion after persisting (so result is saved before ending)
  if (selectedMode === 'mission' && modeInstance.isComplete?.()) { endSession(); return; }

  // Guard: if race/survival ended the session during the await, don't advance
  if (!isSessionActive) return;
  setTimeout(() => nextRound(), 800); // 800ms feels snappier than 1200ms
}
...
async function endSession() {
  if (!isSessionActive) return; // prevent double-call from race/quit + timeout
  isSessionActive = false;
  currentWord = null;
  modeInstance?.stop?.();
