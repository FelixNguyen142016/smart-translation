...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // the word list and resets to the default FSRS filter ("Due Today"), same
  // reset-on-entry behavior Review's startReviewSession() already had before
  // filter pills existed.
  async function renderPractice() {
    _activePracticeMode = 'read-write';
    _practiceAllWords = await getWords();
    _activePracticeFilter = 'due';
    _renderPracticeFilterStrip();
    _applyPracticeFilter();
  }
  ...
  function _showNextFitb() {
    ...
    // as wrong (no push to _practiceWrong) — it just reveals the answer via
  ...
  function _showNextListen() {
    ...
    const nextBtn = document.createElement('button');
    nextBtn.id = 'practice-next-btn'; // see the comment on Read-and-Write's nextBtn above
    nextBtn.className = 'btn-ghost';
    nextBtn.style.cssText = 'padding:10px 20px;font-size:14px;display:none;';
    nextBtn.textContent = 'Next →';
    ...
    // Same shared reveal transition as Read-and-Write's — see its comment.
    function _revealUI() {
      answered = true;
      input.disabled = true;
      checkBtn.style.display = 'none';
    ...
    // Neutral, not counted as wrong — same reasoning as Read-and-Write's skipAnswer().
    function skipAnswer() {
      if (answered) return;
      _revealUI();
      feedback.innerHTML = `<span style="color:#f59e0b;">→ Skipped — the answer was <strong>${escapeHtml(word.text)}</strong></span>`;
    ...
    checkBtn.onclick = checkAnswer;
    skipBtn.onclick  = skipAnswer;
    // stopPropagation fix — see the matching comment on Read-and-Write's
    // input keydown listener above for why this is required.
    input.addEventListener('keydown', e => { if (e.key === 'Enter') { e.stopPropagation(); checkAnswer(); } });
