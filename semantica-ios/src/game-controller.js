...
// ─── Survival visual (Phaser campfire canvas, or the plain bar if it didn't load) ──
// The immersive full-card layout (.immersive-survival, see index.html) only
// applies when the Phaser scene actually mounted — if Phaser failed to load,
// Survival falls back to the same plain-bar/text layout Race mode uses, so
// the .immersive-survival class only ever gets added in that branch.
//
// Visibility/class MUST be applied before mountSurvivalScene()/
// resetSurvivalScene() run: Phaser's Scale.FIT mode measures its parent
// container's size at construction time and doesn't automatically pick up a
// later CSS-driven resize. mountSurvivalScene() is idempotent-once (the
// Phaser.Game instance is only ever constructed the first time Survival is
// played in a session) — if the container were still display:none / missing
// .immersive-survival at that first call, Phaser would lock onto a
// collapsed 0-size measurement and the canvas would render tiny/wrong for
// the rest of the session. resetSurvivalScene() also force-refreshes the
// scale manager as a second line of defense (see its own comment).
function setupSurvivalVisual() {
  const wrap      = document.querySelector('.race-bar-wrap');
  const container = document.getElementById('survival-canvas-container');
  const card      = document.querySelector('.game-play-card');
  if (!container) return;
  if (isSurvivalSceneAvailable()) {
    if (wrap) wrap.style.display = 'none';
    container.style.display = 'block';
    card?.classList.add('immersive-survival');
    mountSurvivalScene('survival-canvas-container');
    resetSurvivalScene();
  } else {
    container.style.display = 'none';
    if (wrap) wrap.style.display = '';
...
function teardownSurvivalVisual() {
  const wrap      = document.querySelector('.race-bar-wrap');
  const container = document.getElementById('survival-canvas-container');
  const card      = document.querySelector('.game-play-card');
  if (container) container.style.display = 'none';
  if (wrap) wrap.style.display = '';
  card?.classList.remove('immersive-survival');
}

// ─── Mission visual (Phaser expedition-path canvas) ────────────────────────
// Mission mode has no bar equivalent to fall back to (#mission-objectives'
...
// so unlike Race/Survival there's no wrap element to toggle here.
//
// Same container-visible-before-mount ordering as setupSurvivalVisual() (see
// its comment for the full Scale.FIT rationale) — applied here from the
// start rather than discovered via a broken screenshot a second time.
function setupMissionVisual() {
  const container = document.getElementById('mission-canvas-container');
  if (!container) return;
  if (isMissionSceneAvailable()) {
    container.style.display = 'block';
    mountMissionScene('mission-canvas-container');
    resetMissionScene();
  } else {
    container.style.display = 'none';
  }
...
function teardownMissionVisual() {
  const container = document.getElementById('mission-canvas-container');
  if (container) container.style.display = 'none';
}

// ─── Quick Ear visual (Phaser radio-dial canvas + the always-shown HTML timer) ──
...
// whenever Quick Ear is active regardless of whether Phaser loaded. The
// canvas is purely an optional cosmetic layer on top of it.
//
// Also toggles the Hint button off (a hint would trivialize a dictation
// test — the whole point is recalling spelling from sound alone) and the
// Replay button on, since both only make sense for this mode.
//
// Same mount-before-visible ordering as setupSurvivalVisual()/
// setupMissionVisual() (see setupSurvivalVisual's comment for the full
// Scale.FIT rationale) — applied here from the start.
function setupQuickEarVisual() {
  const container = document.getElementById('quickear-canvas-container');
  ...
  if (!container) return;
  if (isQuickEarSceneAvailable()) {
    container.style.display = 'block';
    mountQuickEarScene('quickear-canvas-container');
    resetQuickEarScene();
  } else {
    container.style.display = 'none';
  }
...
function teardownQuickEarVisual() {
  const container = document.getElementById('quickear-canvas-container');
  const timerWrap = document.getElementById('quickear-timer-wrap');
  const hintBtn   = document.getElementById('btn-hint');
  const replayBtn = document.getElementById('btn-replay-audio');
...
async function startSession() {
  ...
  if (selectedMode === 'race') {
    ...
    const shuffled = [...words].sort(() => Math.random() - 0.5);
    quickEarQueue = quickEarWordLimit === Infinity ? shuffled : shuffled.slice(0, quickEarWordLimit);
  ...
  if (selectedMode === 'race') {
    setupRaceVisual();
    teardownSurvivalVisual();
    teardownMissionVisual();
    teardownQuickEarVisual();
  } else if (selectedMode === 'survival') {
    teardownRaceVisual();
    setupSurvivalVisual();
    teardownMissionVisual();
...
function useHint() {
  // Quick Ear has no hint mechanism (the button is hidden — see
  // setupQuickEarVisual) — any hint would reveal spelling info a dictation
  // test is meant to test recall of. Defensive guard in case this is ever
  // reachable from somewhere other than the hidden button.
