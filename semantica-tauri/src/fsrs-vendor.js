// renderer/fsrs-vendor.js
//
// Vendored copy of ts-fsrs v5.4.1 (https://www.npmjs.com/package/ts-fsrs),
// the reference JS/TS implementation of FSRS (Free Spaced Repetition
// Scheduler) — MIT licensed, https://github.com/open-spaced-repetition/ts-fsrs
//
// Why vendored instead of imported from a CDN (esm.sh/jsdelivr/unpkg):
// this project has no bundler (plain <script type="module"> ES modules,
// same as every other file in src/), and ts-fsrs ships ESM/CJS/UMD builds
// that *could* be imported directly from a CDN URL at runtime. But unlike
// Phaser (used for the Game tab's cosmetic Race visual, which already has a
// graceful `typeof Phaser === 'undefined'` fallback), FSRS backs Review
// Mode's actual due-date scheduling — there's no sensible fallback if the
// network is unavailable or a CDN is unreachable, and the app should work
// identically offline on both Windows and macOS without depending on any
// particular CDN's uptime or regional availability. Vendoring removes that
// entire class of risk: this file is fetched once (unpkg.com/ts-fsrs@5.4.1
// /dist/index.mjs, the unminified ESM build) and committed to the repo, so
// scheduling behavior is 100% deterministic and offline-capable everywhere.
//
// This is an unmodified, faithful copy of the published algorithm and
// default parameter weights — do not hand-edit the FSRS math below. To
// upgrade, refetch the same URL with a newer version pin and diff.
//
// Copyright (c) ts-fsrs contributors — MIT License.
